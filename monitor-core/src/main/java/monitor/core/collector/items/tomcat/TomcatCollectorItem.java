            ...
            try {
                if (this.httpObjectName == null) {
                    ...
                    if (CollectionUtils.isEmpty(catalinaThreadPoolObjectNames)) {
                        ++this.isNullTimes;
                        LOGGER.log(Level.WARNING, "mBeanServer [Catalina:type=ThreadPool,name=*] not found, it may be not a tomcat container ");
                        return collectDatas;
                    }
                ...
                LOGGER.log(Level.SEVERE, "failed to constructItemRow", var6);
                return collectDatas;
            }
        }
        return collectDatas;
